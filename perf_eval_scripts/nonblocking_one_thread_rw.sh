#!/usr/bin/env bash
TEST_NAME="nonblocking_one_thread_rw";
...
for i in $SIZES; do
	...
	echo "$TEST_EXE_DIR/a.out 1 1 $NUM_OPS $NUM_OPS 0 0 1 $i $DEFAULT_MAX_STORAGE  > $EVAL_DIR/${TEST_NAME}_$i.dat"
	sudo $TEST_EXE_DIR/a.out 1 1 $NUM_OPS $NUM_OPS 0 0 1 $i $DEFAULT_MAX_STORAGE  > $EVAL_DIR/${TEST_NAME}_$i.dat
...
#	SIZES="8191"
    ...
    set title "One writer/One reader (non-blocking)"
