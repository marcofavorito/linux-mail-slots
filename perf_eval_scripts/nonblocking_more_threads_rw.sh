#!/usr/bin/env bash
TEST_NAME="nonblocking_more_threads_rw"
...
do
	...
	echo "sudo $TEST_EXE_DIR/a.out $cur_no_threads $cur_no_threads $cur_num_ops $cur_num_ops 0 0 1 $DEFAULT_MAX_DATAUNIT_LEN $DEFAULT_MAX_STORAGE  > $EVAL_DIR/${TEST_NAME}_$cur_no_threads.dat"
	sudo $TEST_EXE_DIR/a.out $cur_no_threads $cur_no_threads $cur_num_ops $cur_num_ops 0 0 1 $DEFAULT_MAX_DATAUNIT_LEN $DEFAULT_MAX_STORAGE  > $EVAL_DIR/${TEST_NAME}_$cur_no_threads.dat
...
#	NUMS_OPS="100000"
    ...
    set title "More writers/More readers (non-blocking)"
